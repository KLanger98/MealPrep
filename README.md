...
Seed a fresh local environment (dev server must be running for the sync
step): `./scripts/seed.sh` — uploads `recipes/*` to local R2 + POST /sync.
It accepts `--remote` to target the real bucket.
