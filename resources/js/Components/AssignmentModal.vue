...
<template>
    <Teleport to="body">
        <div v-if="show" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" @click.self="emit('close')">
            <div class="w-full max-w-md rounded-xl bg-white p-5 shadow-xl dark:bg-stone-900">
                ...
                <form class="mt-4 space-y-4" @submit.prevent="submit">
                    <div v-if="!isEdit">
                        <label class="block text-sm font-medium text-stone-700 dark:text-stone-300">Recipe</label>
                        <input
                            v-model="search"
                            type="search"
                            placeholder="Search…"
                            class="mt-1 w-full rounded-lg border border-stone-300 px-3 py-1.5 text-sm dark:border-stone-700 dark:bg-stone-950"
                        />
                        <div class="mt-2 max-h-40 space-y-1 overflow-y-auto rounded-lg border border-stone-200 p-1 dark:border-stone-700">
                            <button
                                v-for="recipe in filteredRecipes"
                                :key="recipe.id"
                                type="button"
                                class="flex w-full items-center justify-between rounded-md px-2 py-1.5 text-left text-sm"
                                :class="form.recipe_id === recipe.id ? 'bg-green-600 text-white' : 'hover:bg-stone-100 dark:hover:bg-stone-800'"
                                @click="form.recipe_id = recipe.id"
                            >
                                <span>{{ recipe.title }}</span>
                                <span class="text-xs opacity-70">{{ recipe.servings }} serv · {{ recipe.type }}</span>
                            </button>
                            <p v-if="!filteredRecipes.length" class="px-2 py-1.5 text-sm text-stone-400 dark:text-stone-500">No matches.</p>
                        </div>
                    </div>
                    ...
                    <div class="flex gap-3">
                        <label class="flex-1 text-sm font-medium text-stone-700 dark:text-stone-300">
                            Slot
                            <select v-model="form.slot" class="mt-1 w-full rounded-lg border border-stone-300 px-2 py-1.5 text-sm dark:border-stone-700 dark:bg-stone-950 capitalize">
                        ...
                        <label class="flex-1 text-sm font-medium text-stone-700 dark:text-stone-300">
                            ...
                            <input
                                ...
                                max="20"
                                step="any"
                                class="mt-1 w-full rounded-lg border border-stone-300 px-2 py-1.5 text-sm dark:border-stone-700 dark:bg-stone-950"
                            />
                        </label>
                    </div>

                    <p v-if="totalServings" class="text-xs text-stone-500 dark:text-stone-400">
                    ...
                    <div v-if="!isEdit" class="flex gap-3">
                        <label class="flex-1 text-sm font-medium text-stone-700 dark:text-stone-300">
                            From
                            <input v-model="form.start_date" type="date" required class="mt-1 w-full rounded-lg border border-stone-300 px-2 py-1.5 text-sm dark:border-stone-700 dark:bg-stone-950" />
                        </label>
                        <label class="flex-1 text-sm font-medium text-stone-700 dark:text-stone-300">
                            To
                            <input v-model="form.end_date" type="date" required :min="form.start_date" class="mt-1 w-full rounded-lg border border-stone-300 px-2 py-1.5 text-sm dark:border-stone-700 dark:bg-stone-950" />
                        </label>
                    </div>

                    <p v-if="!isEdit" class="text-xs text-stone-500 dark:text-stone-400">
                        Covering multiple days means one cook (one batch) eaten across those days.
                    </p>

                    <div class="flex justify-end gap-2 pt-2">
                        <button type="button" class="rounded-lg px-3 py-1.5 text-sm text-stone-600 hover:bg-stone-100 dark:text-stone-300 dark:hover:bg-stone-800" @click="emit('close')">
                            Cancel
                        </button>
                        <button
                            type="submit"
